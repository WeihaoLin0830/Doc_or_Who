...
# Meses en español y sus equivalentes núméricos
_MESES: dict[str, str] = {
...
def _normalize_date(raw: str, fmt: str) -> str | None:
    """
    Convierte una fecha raw al formato ISO (YYYY-MM-DD, YYYY-MM o YYYY).
    Devuelve None si no se puede normalizar.
    """
    raw = raw.strip()
    try:
        if fmt == "dmy_slash":   # DD/MM/YYYY o D/M/YYYY
            parts = raw.split("/")
            return f"{int(parts[2]):04d}-{int(parts[1]):02d}-{int(parts[0]):02d}"
        if fmt == "dmy_dash":    # DD-MM-YYYY o D-M-YYYY
            parts = raw.split("-")
            return f"{int(parts[2]):04d}-{int(parts[1]):02d}-{int(parts[0]):02d}"
        if fmt == "iso":         # YYYY-MM-DD — ya es ISO
            return raw
        if fmt == "dmy_text":    # "10 de enero de 2025" / "10 enero 2025"
            m = re.match(
                r"(\d{1,2})\s+(?:de\s+)?([a-záéíóúü]+)\s+(?:de\s+)?(\d{4})",
                raw, re.IGNORECASE,
            )
            if m:
                day, month_name, year = m.groups()
                mes = _MESES.get(month_name.lower())
                if mes:
                    return f"{int(year):04d}-{mes}-{int(day):02d}"
        if fmt == "my_text":     # "enero de 2025" / "enero 2025"
            m = re.match(
                r"([a-záéíóúü]+)\s+(?:de\s+)?(\d{4})",
                raw, re.IGNORECASE,
            )
            if m:
                ...
                mes = _MESES.get(month_name.lower())
                if mes:
                    return f"{int(year):04d}-{mes}"
        if fmt == "year_only":   # Solo año
...
def _extract_dates(text: str) -> list[str]:
    """
    Extrae fechas del texto, las normaliza a formato ISO (YYYY-MM-DD / YYYY-MM / YYYY)
    y devuelve la lista ordenada y deduplicada.
    """
    hits: list[tuple[str, str]] = []  # (raw, fmt)
    ...
    rules: list[tuple[str, str]] = [
        ...
        (r"\d{1,2}\s+de\s+(?:" + "|".join(_MESES) + r")\s+de\s+\d{4}",           "dmy_text"),
        (r"\d{1,2}\s+(?:" + "|".join(_MESES) + r")\s+de\s+\d{4}",               "dmy_text"),
        (r"\d{1,2}\s+(?:" + "|".join(_MESES) + r")\s+\d{4}",                    "dmy_text"),
        (r"(?:" + "|".join(_MESES) + r")\s+de\s+\d{4}",                          "my_text"),
        (r"(?:" + "|".join(_MESES) + r")\s+\d{4}",                               "my_text"),
    ...
    for pattern, fmt in rules:
        for m in re.finditer(pattern, text, re.IGNORECASE):
            raw = m.group(0).strip()
            if raw.lower() not in seen_raw:
                seen_raw.add(raw.lower())
                hits.append((raw, fmt))

    # Normalizar y deduplicar por valor ISO
    normalized: dict[str, str] = {}  # iso_value → iso_value
    for raw, fmt in hits:
        iso = _normalize_date(raw, fmt)
        if iso and iso not in normalized:
            normalized[iso] = iso
